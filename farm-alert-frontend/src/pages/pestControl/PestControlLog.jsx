...
export default function PestControlLog() {
  ...
  // CSV Export
  // ---------------------------------------------------------------------------
  const PEST_CSV_COLUMNS = [
    { label: 'Date',             key: row => row.date_of_intervention ? new Date(row.date_of_intervention).toLocaleDateString('en-PH') : '' },
    { label: 'Farm',             key: row => row.farms?.farm_name ?? '' },
    { label: 'Barangay',         key: row => row.farms?.barangays?.barangay_name ?? '' },
    { label: 'Pest Type',        key: 'pest_type' },
    { label: 'Treatment Applied',key: 'treatment_applied' },
    { label: 'Encoded By',       key: row => row.users?.full_name ?? '' },
    { label: 'Notes',            key: 'notes' },
  ];

  const handleExport = () => {
    const date = new Date().toISOString().slice(0, 10);
    exportToCsv(`pest_control_logs_${date}.csv`, logs, PEST_CSV_COLUMNS);
  };
